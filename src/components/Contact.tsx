...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const GOOGLE_FORM_ACTION_URL = "https://docs.google.com/forms/d/e/1FAIpQLSdZVJnLDE4upMGUlNlyLl-Ob9ay4ob1DlorHUVfVJqCadN6Xw/formResponse";
    const NAME_ENTRY_ID = "entry.538260938";    // Replace with your Name entry ID
    const EMAIL_ENTRY_ID = "entry.1636873249";   // Replace with your Email entry ID
    const MESSAGE_ENTRY_ID = "entry.510255708"; // Replace with your Message entry ID

    const formData = new URLSearchParams();
    formData.append(NAME_ENTRY_ID, formState.name);
    formData.append(EMAIL_ENTRY_ID, formState.email);
    formData.append(MESSAGE_ENTRY_ID, formState.message);

    try {
      await fetch(GOOGLE_FORM_ACTION_URL, {
        method: 'POST',
        mode: 'no-cors', // This is required to bypass Google's cross-origin restrictions
        headers: {
      ...
      // Because of 'no-cors', we don't get a readable response back.
      // If the fetch didn't throw a network error, we assume it worked.
      setIsSuccess(true);
    ...
    <section className="relative z-20 bg-[#0a0a0a] py-32 px-4 md:px-12 overflow-hidden" id="contact">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          ...
          >
            ...
            <p className="text-gray-400 text-lg mb-10 max-w-md leading-relaxed">
              Hey, would love to connect and brainstorm to create and ship a production grade AI system
            </p>

            <div className="flex flex-col gap-6 mb-12">
              <a href="mailto:ntsalunke4@gmail.com" className="flex items-center gap-4 text-gray-300 hover:text-white transition-colors group">
                ...
                <span className="text-lg">ntsalunke4@gmail.com</span>
              </a>
        ...
        <footer className="mt-24 pt-8 border-t border-white/5 text-center text-gray-500 font-mono text-sm">
          <p>&copy; {new Date().getFullYear()} Nikhil Salunke</p>
        </footer>
      </div>
    </section>
  );
